...
namespace json {
  ...
  std::tuple<JSONToken, int, std::string> lex_string(std::string raw_json, int original_index) {
  ...
  std::tuple<JSONToken, int, std::string> lex_number(std::string raw_json, int original_index) {
    int index = original_index;
  ...
  std::tuple<JSONToken, int, std::string> lex_keyword(std::string raw_json, std::string keyword, JSONTokenType type, int original_index) {
    int index = original_index;
  ...
  std::tuple<JSONToken, int, std::string> lex_null(std::string raw_json, int index) {
    return lex_keyword(raw_json, "null", JSONTokenType::Null, index);
  }

  std::tuple<JSONToken, int, std::string> lex_true(std::string raw_json, int index) {
    return lex_keyword(raw_json, "true", JSONTokenType::Boolean, index);
  }

  std::tuple<JSONToken, int, std::string> lex_false(std::string raw_json, int index) {
  ...
  std::string format_parse_error(std::string base, JSONToken token) {
    std::ostringstream s;
    s << "Unexpected token '" << token.value << "', type '" << token.type << "', index ";
    s << std::endl << base;
    return format_error(s.str(), *token.full_source, token.location);
  }

  std::tuple<std::vector<JSONValue>, int, std::string> parse_array(std::vector<JSONToken> tokens, int index) {
	...
	if (t.value == ",") {
	  ...
	  return {{}, index, format_parse_error("Expected comma after element in array", t)}; 
	}
      }
    ...
    return {{}, index, format_parse_error("Unexpected EOF while parsing array", tokens[index])};
  }

  std::tuple<std::map<std::string, JSONValue>, int, std::string> parse_object(std::vector<JSONToken> tokens, int index) {
	...
	if (t.value == ",") {
	  ...
	  return {{}, index, format_parse_error("Expected comma after element in object", t)}; 
	} else {
	  return {{}, index, format_parse_error("Expected key-value pair or closing brace in object", t)};
	}
      }
	...
	return {{}, index, format_parse_error("Expected string key in object", t)};
      }
      index = new_index;
      t = tokens[index];

      if (!(t.type == JSONTokenType::Syntax && t.value == ":")) {
	return {{}, index, format_parse_error("Expected colon after key in object", t)};
      }
      index++;
      t = tokens[index];
  ...
  std::tuple<JSONValue, int, std::string> parse(std::vector<JSONToken> tokens, int index) {
